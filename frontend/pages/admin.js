...
const SWR1 = () => {
    const [shops, setshops] = useState({ list: [{ id: 1, name: "คืนนี้ผมต้องเล่าเรื่องผีให้คุณแม่มดฟัง", rental:30, Category:"นวนิยาย" },] })
    ...
    const getshop = async (id) => {
        ...
        setshop({ id: shop.data.id, name: shop.data.name, rental: shop.data.rental,  Category: shop.data.Category, imageurl: shop.data.imageurl})
    }
        ...
        if (shops && shops.length)
            return shops.map((shop, index) =>
                <li className={styles.listItem} key={index}>
                    ...
                    rental:{(shop) ? shop.rental : 0}
                    <h6>Category:{(shop) ? shop.Category : 0}</h6>
    ...
    const addshop = async (  name, rental, Category, imageurl) => {
        let shops = await axios.post(URL, {  name, rental, Category, imageurl })
        setshops(shops.data)
    }
    ...
    const updateshop = async (id) => {
        const result = await axios.put(`${URL}/${id}`, { id, name, rental, Category, imageurl })
...
<link href="https://fonts.googleapis.com/css2?family=Itim&display=swap" rel="stylesheet"></link>
        ...
        <link rel="preconnect" href="https://fonts.gstatic.com"/>
<link href="https://fonts.googleapis.com/css2?family=Itim&display=swap" rel="stylesheet"></link>
        <h2>Book rental shop</h2>
        <link rel="preconnect" href="https://fonts.gstatic.com"/>
<link href="https://fonts.googleapis.com/css2?family=Itim&display=swap" rel="stylesheet"></link>


        <ul className={styles.list}  >{printshops()}</ul>
        selected order: {shop.name} {shop.rental}  {shop.Category}  {shop.imageurl}
        <h2>Add Order</h2>
        <ul className={styles.formadd} >
           
            name:<input type="text" onChange={(e) => setname(e.target.value)} /> <br />
            rental:<input type="number" onChange={(e) => setrental(e.target.value)} /> <br />
        ...
        imageurl:<input type="Linkd" onChange={(e) => setimageurl(e.target.value)} /> <br />
            <button className={styles.byttonadd} onClick={() => addshop(name,  rental, Category, imageurl)}>Add New Book</button>
